...
\begin{center}
\begin{tabular}{l}
\textbf{$\leftarrow$\patternname{Carrying capacity}: It is useful to record obstacles, threats, and outstanding challenges.}\\
\textbf{$\leftarrow$\patternname{A specific project}: We want to stay connected to concrete action, not just theory.}\\
\textbf{$\leftarrow$\patternname{Heartbeat}: A project that is no longer rewarding may be put on the back burner.}\\
\end{tabular}
\end{center}
