...
% Appendix
\elecappendix
\section{Reading between the lines, etymologically}

I want to insert a philosophical note ``between the lines'' of our
article on Patterns of Peeragogy.  First, an etymological fragment,
for context.
...
It's another interesting coincidence that ``paragogy'' is a Greek
word, meaning production.  I always liked the implied double meaning
that paragogy was about both ``peer learning'' and ``production.''
The gloss ``peer produced peer learning'' puts the emphasis on
\emph{peer learning}.  It would be better if the meaning was split
evenly between peer learning and (possibly ``peer'') production.
With ``peeragogy'' the dual-language pun is gone entirely, but it
would be good if ``production'' aspects still got fair play.  With
these comments in mind, here is a fairly profound quote:
...
subtle and subterranean differential mechanisms. \citeadx{deleuze2006nietzsche}
\end{quotation}

Here we have ``para-'' in both its Greek and its Latin forms, opposed
but subtly so.  Gilles Deleuze is saying that
``thesis/antithesis/synthesis'' isn't how consciousness comes into
being \citeadx[p. 48]{hughes2009deleuze}.  Rather, consciousness and
being itself feels its way forward, distinguishing the present from
the past, creating new forms and re-creating beings as it goes.  So
...
This Deleuzian perspective has practical implications.  In the way of
getting into a tangle with things and sorting them out (as we're doing
in the Peeragogy project) we are ``doing philosophy'' in the Deleuzian
or Bergsonian way.  In connection with ``patterns,'' we're not just
thinking of \emph{abstract} patterns, but patterns that are contingent
on what we do, and even patterns that -- when we distinguish them --
shape who we are.

Something of this dynamical view is developed by Christian Kohls in
\citeadx{kohls2010structure,kohls2011structure}.
